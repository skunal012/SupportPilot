...
> **recruiter-first** (pitch → the three behaviours → architecture diagram → eval
> results → *then* the developer run-guide), a **mermaid diagram** of the query
> lifecycle
...
**1. README, reordered.** New top section is the pitch followed by *"Three
questions, three behaviours"* — a list that maps one-to-one onto the three paths a
question can take (grounded answer / tool call / escalation), and onto the three
beats of the GIF once it is recorded. The existing prerequisites / Docker /
local-dev sections were kept verbatim and moved below — they were never the
problem.
...
> **Status: storyboarded, not recorded.** The README shipped with an
> `![...](docs/demo.gif)` line pointing at a file that was never created, so the
> hero rendered as a broken image on GitHub. The line has been removed and the
> section beneath it rewritten to stand on its own prose. Recording is deferred
> until **after Day 15**, because hybrid retrieval changes the citations and
> scores that appear on screen in beats 1 and 3 — recording now would mean
> recording twice. The storyboard below stays valid; only beat 1's citation list
> needs re-checking once retrieval changes.
